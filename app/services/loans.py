...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    # Fetch all unreturned loans of this member
    ...
    for loan in unreturned_loans:
        if loan.book_id == book.id:
            raise HTTPException(status_code=409, detail="Member already has an unreturned copy of this book")

    # 5. 409 member is at their tier's loan limit
    tier_limit = TIER_LOAN_LIMIT.get(member.tier)
    if tier_limit is not None and len(unreturned_loans) >= tier_limit:
        raise HTTPException(status_code=409, detail=f"Member reached tier loan limit of {tier_limit}")
